...
void tryConnectOrPortal(){
  ...
  wm.setTimeout(300);wm.startConfigPortal("Zeen AI Plug");
}
...
void loop(){
  ...
  /// ---------- ONLINE RUN WITH BULB ANIMATION ----------
else if(currentState==STATE_ONLINE){
    SinricPro.handle();
    digitalWrite(WIFI_LED, WiFi.status()==WL_CONNECTED ? HIGH : LOW);
    handleSetButton();

    if(oledReady && millis() - lastAnimMillis > 450){
        lastAnimMillis = millis();
        display.clearDisplay();

        int cx = SCREEN_WIDTH / 2;
        int cy = 36;
        bool halo = (millis() / 450) % 2; // blink halo every 450ms

        // Draw bulb outline
        display.drawCircle(cx, cy, 12, SSD1306_WHITE);
        display.fillRect(cx-5, cy+12, 10, 5, SSD1306_WHITE); // bulb base
        ...
        // else system OFF -> bulb off (only outline + base)
        display.display();
    }
}
